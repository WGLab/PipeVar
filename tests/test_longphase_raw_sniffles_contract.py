...
class RawSnifflesLongPhaseContractTests(unittest.TestCase):
    ...
    def test_only_longphase_modules_adopt_the_new_image(self):
        expected = {
        ...
        actual = {
            ...
            if "longphase_0.2.30" in path.read_text(encoding="utf-8")
        }
    ...
    def test_single_sample_phases_raw_sniffles_but_curates_evidence(self):
        ...
        self.assertRegex(
            source,
            r"longphase\(bam,ANNOVAR\.out\.vcf_output,sniffles\.out,PhenoSV\.out,",
        )
    ...
    def test_batch_phasing_is_proband_keyed_and_annotation_remains_curated(self):
        ...
        self.assertIn(
            "sniffles_for_phasing.join(join_vcf_bam, failOnMismatch: true, failOnDuplicate: true)",
            source,
        )
        self.assertIn(
    ...
    def test_light_longphase_routes_also_phase_raw_sniffles(self):
        ...
        self.assertRegex(single, r"longphase\(bam,ANNOVAR\.out\.vcf_output,sniffles\.out,PhenoSV\.out,")
