...
chr1\t1000\tCOMMON_DEL\tN\t<DEL>\t.\tMixed\tSVTYPE=DEL;END=2000;SVLEN=-1001;AF=0.02;SUPPORT=12
chr1\t5000\tCOMMON_INS\tA\tATTTT\t.\tPASS\tSVTYPE=INS;END=5000;SVLEN=4;AF=0.05;SUPPORT=20
chr1\t7000\tCOMMON_LOW_AF\tN\t<DUP>\t.\tPASS\tSVTYPE=DUP;END=7800;SVLEN=801;AF=0.005;SUPPORT=2
chr2\t9000\tCOMMON_CHR2\tN\t<DEL>\t.\tPASS\tSVTYPE=DEL;END=9400;SVLEN=-401;AF=0.03;SUPPORT=8
chr1\t12000\tCOMMON_BND\tN\tN[chr2:13000[\t.\tPASS\tSVTYPE=BND;END=12000;SVLEN=1;AF=0.10;SUPPORT=9
"""
...
class FilterCommonSVsImageSmokeTest(unittest.TestCase):
    ...
    def test_common_del_removed_and_rare_del_retained(self):
        sample = patient_vcf(
            "chr1\t1100\tPAT_COMMON_DEL\tN\t<DEL>\t.\tPASS\tSVTYPE=DEL;END=1900;SVLEN=-801\tGT\t0/1\n"
            "chr1\t3000\tPAT_RARE_DEL\tN\t<DEL>\t.\tPASS\tSVTYPE=DEL;END=3300;SVLEN=-301\tGT\t0/1\n"
        )
    ...
    def test_reciprocal_overlap_threshold_boundary(self):
        sample = patient_vcf(
            "chr1\t2501\tBELOW_RO\tN\t<DEL>\t.\tPASS\tSVTYPE=DEL;END=3501;SVLEN=-1001\tGT\t0/1\n"
            "chr1\t1400\tABOVE_RO\tN\t<DEL>\t.\tPASS\tSVTYPE=DEL;END=2200;SVLEN=-801\tGT\t0/1\n"
        )
    ...
    def test_common_insertion_sequence_removed_but_different_sequence_retained(self):
        sample = patient_vcf(
            "chr1\t5000\tINS_SAME\tA\tATTTT\t.\tPASS\tSVTYPE=INS;END=5000;SVLEN=4\tGT\t0/1\n"
            "chr1\t5000\tINS_DIFF\tA\tAGGGG\t.\tPASS\tSVTYPE=INS;END=5000;SVLEN=4\tGT\t0/1\n"
        )
        kept, removed, _summary = self.run_filter(sample)
    ...
    def test_chromosome_normalization_and_low_af_skip(self):
        sample = patient_vcf(
            "2\t9050\tNO_CHR_DEL\tN\t<DEL>\t.\tPASS\tSVTYPE=DEL;END=9350;SVLEN=-301\tGT\t0/1\n"
            "chr1\t7100\tLOW_AF_DUP\tN\t<DUP>\t.\tPASS\tSVTYPE=DUP;END=7700;SVLEN=601\tGT\t0/1\n"
        )
        kept, removed, _summary = self.run_filter(sample)
    ...
    def test_bnd_retained_and_header_only_kept_vcf_valid(self):
        sample = patient_vcf(
            "chr1\t12000\tPAT_BND\tN\tN[chr2:13000[\t.\tPASS\tSVTYPE=BND;END=12000;SVLEN=1\tGT\t0/1\n"
            "chr1\t1100\tONLY_COMMON\tN\t<DEL>\t.\tPASS\tSVTYPE=DEL;END=1900;SVLEN=-801\tGT\t0/1\n"
        )
        kept, removed, _summary = self.run_filter(sample)
