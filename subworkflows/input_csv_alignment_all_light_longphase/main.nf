...
workflow INPUT_CSV_ALIGNMENT_ALL_LIGHT_LONGPHASE {
	...
	phen2gene_result=multi_phen2gene(input_bam_no_bam)
        ...
        if ( target == "yes" ) {
	                annovar_input=nanocaller_result.join(phen2_gene_bed, failOnMismatch: true, failOnDuplicate: true).map { out_prefix, vcf_file, bed_file -> tuple(out_prefix, vcf_file, bed_file) }
        }
        else {
	...
	join_vcf_bam_sv=sniffles_result.join(annovar_sv_result, failOnMismatch: true, failOnDuplicate: true).join(join_vcf_bam, failOnMismatch: true, failOnDuplicate: true)
	join_vcf_bam_phenosv=phenosv_result.join(join_vcf_bam_sv, failOnMismatch: true, failOnDuplicate: true)
	join_vcf_bam_rankscore=rankscore_result.join(join_vcf_bam_phenosv, failOnMismatch: true, failOnDuplicate: true)
	join_vcf_bam_rankvar=rankvar_result.join(join_vcf_bam_rankscore, failOnMismatch: true, failOnDuplicate: true)
	...
	join_vcf_bam_rankvar_hpo=join_vcf_bam_rankvar.join(input_bam_hpo_age, failOnMismatch: true, failOnDuplicate: true)
