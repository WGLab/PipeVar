...
workflow INPUT_CSV_NGS_SNP {
	...
	phen2gene_result=multi_phen2gene(input_bam_no_bam)
        if ( target == "yes" ) {
                ...
                deepvariant_result=multi_deepvariant(input_bam_with_bam,ref_fa,phen2_gene_bed)
        }
        else {
                deepvariant_result=multi_deepvariant(input_bam_with_bam,ref_fa,target)
        }
        deepvariant_result_annovar=deepvariant_result.join(input_bam_no_bam)
        annovar_result=multi_annovar(deepvariant_result_annovar)
