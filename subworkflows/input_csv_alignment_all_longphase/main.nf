...
workflow INPUT_CSV_ALIGNMENT_ALL_LONGPHASE {
	...
	phen2gene_result=multi_phen2gene(input_bam_no_bam)
        if ( target == "yes" ) {
                ...
                clair3_result=multi_clair3(input_bam_with_bam,ref_fa,phen2_gene_bed)
        }
        else {
                clair3_result=multi_clair3(input_bam_with_bam,ref_fa,target)
        }
	annovar_result=multi_annovar(clair3_result)
	...
	multi_longphase(join_vcf_bam_rankvar,ref_fa)
