// Batch mobile-element insertion calling for short-read BAM alignments using SCRAMBLE.
process multi_scramble {
	...
	output:
	tuple val(out_prefix), path("${out_prefix}_scramble.vcf"), emit: vcf
	tuple val(out_prefix), path("${out_prefix}_scramble.clusters.txt"), optional: true, emit: clusters
	tuple val(out_prefix), path("${out_prefix}_scramble.MEIs.txt"), optional: true, emit: meis
