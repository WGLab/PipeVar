...
- Enable SCRAMBLE with `--scramble yes` only for short-read BAM runs.
- `--scramble yes` is valid with `--mode sv` or when `--mode` is omitted.
- `--scramble yes` is rejected for VCF-only input, CRAM input, `--type ont|pacbio`, and `--mode snp`.
- The SCRAMBLE container must already include `cluster_identifier`, `SCRAMble.R`, and `MEI_consensus_seqs.fa`.
